			...
			<meta itemprop="interactionCount" content="UserComments:{$commentsCount}">
				...
				<ul>
					<li id="date">
						...
						{* Written on *}
						<span class="day">
							<time itemprop="datePublished" datetime="{$item.publish_on|date:'Y-m-d\TH:i:s'}">{$item.publish_on|date:'d':{$LANGUAGE}}</time></span><br/>{$item.publish_on|date:'m':{$LANGUAGE}}

					</li>
					<li class="background commentCount">
						{* Comments *}
						{option:!comments}<a href="{$item.full_url}#{$actComment}" itemprop="discussionUrl">0</a>{/option:!comments}
						{option:comments}
							<span class="commentNumber">
								{option:blogCommentsMultiple}
									<a href="{$item.full_url}#{$actComments}" itemprop="discussionUrl">{$commentsCount}</a>
								{/option:blogCommentsMultiple}
							</span>
